...
def generate_performance_report(results: Dict, quantizer_configs: List) -> str:
    ...
    for config in quantizer_configs:
        ...
        if name in results and len(results[name]["R"]) > 0:
            ...
            report.append(f"\n{name}:")
            report.append(
                f"  Rate range: {np.min(rates):.3f} - {np.max(rates):.3f} bits/dim"
            )
            report.append(
                f"  Distortion range: {np.min(errors):.6f} - {np.max(errors):.6f}"
            )
    ...
    if convergence:
        ...
        for name, conv_data in convergence.items():
            report.append(f"\n{name}:")
            report.append(
                f"  Rate increment: {conv_data['rate_increment_mean']:.3f} ± {conv_data['rate_increment_std']:.3f}"
            )
            report.append(
                f"  Error decay rate: {conv_data['exponential_decay_rate']:.3f}"
            )
