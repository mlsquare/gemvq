...
class ColumnwiseMatVecProcessor(ABC):
    ...
    def _detect_sparsity(self, x: np.ndarray) -> Tuple[List[int], float]:
        ...
        non_zero_indices = [i for i, val in enumerate(x) if abs(val) > self.sparsity_threshold]
    ...
    def get_compression_stats(self) -> Dict[str, float]:
        ...
        self.stats['compression_ratio'] = original_bits / encoded_bits if encoded_bits > 0 else float('inf')
        ...
        self.stats['memory_usage_mb'] = encoded_size * 4 / (1024 * 1024)  # Assuming 4 bytes per element
