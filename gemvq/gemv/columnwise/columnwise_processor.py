    ...
    def process(self, matrix: np.ndarray, vector: np.ndarray) -> np.ndarray:
        ...
        for block_idx in range(n_blocks):
            ...
            for col_idx in range(start_col, end_col):
                ...
                decoded_column = self._decode_column(
                    column_encodings, quantizer, decoding_method
                )

                # Add contribution to result
                result += (
                    block_x[col_idx - start_col] * decoded_column[: self.original_m]
                )

        return result[: self.original_m]

    def _decode_column(
        self, column_encodings: List, quantizer: HNLQ, decoding_method: str
    ) -> np.ndarray:
