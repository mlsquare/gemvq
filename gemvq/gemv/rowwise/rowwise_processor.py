...
from ...quantizers.utils import (closest_point_A2, closest_point_Dn, closest_point_E8,
    ...
    def process(self, matrix: np.ndarray, vector: np.ndarray) -> np.ndarray:
        ...
        for block_idx in range(m_blocks):
            ...
            for row_idx in range(start_row, end_row):
                ...
                decoded_row = self._decode_row(row_encodings, quantizer, decoding_method)
                
                # Compute dot product with vector
                dot_product = np.dot(decoded_row[:self.original_n], padded_x[:self.original_n])
