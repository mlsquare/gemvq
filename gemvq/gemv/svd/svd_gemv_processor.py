...
class SVDGEMVProcessor(GEMVProcessor):
    ...
    def _compute_svd(self):
        ...
        self.stats["svd_error"] = np.linalg.norm(
            self.original_matrix - reconstructed, "fro"
        )
        ...
        for i in range(m):
            ...
            for j in range(0, n, self.dimension):
                ...
                if len(chunk) < self.dimension:
                    chunk = np.pad(
                        chunk, (0, self.dimension - len(chunk)), mode="constant"
                    )
    ...
    def _decode_matrix(
        self, quantized_matrix: Dict, decoding_method: str
    ) -> np.ndarray:
        ...
        for i, row_encodings in enumerate(encodings):
            for j, (encoding, T) in enumerate(row_encodings):
                if decoding_method == "full":
                    ...
                    chunk = quantizer.decode_coarse_to_fine(
                        encoding, T, with_dither=False
                    )
    ...
    def _decode_vector(
        self, quantized_vector: Dict, decoding_method: str
    ) -> np.ndarray:
    ...
    def process(self, matrix: np.ndarray, vector: np.ndarray) -> np.ndarray:
        ...
        if len(vector) != self.original_n:
            raise ValueError(
                f"Vector length {len(vector)} != matrix columns {self.original_n}"
            )
