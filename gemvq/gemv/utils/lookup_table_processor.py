...
class LookupTableProcessor(ColumnwiseMatVecProcessor):
    ...
    def compute_matvec(
        ...
        if self.table_strategy == "layer_wise_histogram":
            result = self._compute_layer_wise_histogram(
                padded_x, decoding_depths, sparsity_pattern
            )
        elif self.table_strategy == "inner_product":
            result = self._compute_inner_product_tables(
                padded_x, decoding_depths, sparsity_pattern
            )
        elif self.table_strategy == "hybrid":
    ...
    def _compute_layer_wise_histogram(
        self, x: np.ndarray, decoding_depths: List[int], sparsity_pattern: List[int]
    ) -> np.ndarray:
        """
        ...
        for m in range(self.M):
            ...
            layer_histogram = self._compute_layer_histogram(
                x, m, decoding_depths, sparsity_pattern
            )
    ...
    def _lookup_inner_product(
        self, x_block: np.ndarray, decoded_block: np.ndarray
    ) -> Optional[np.ndarray]:
        """
    ...
    def get_table_stats(self) -> Dict[str, Union[int, float]]:
        ...
        return {
            ...
            "table_memory_mb": total_table_size
            * 8
            / (1024 * 1024),  # Assuming 8 bytes per element
        }
