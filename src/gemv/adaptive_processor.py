...
class AdaptiveProcessor(ColumnwiseMatVecProcessor):
    ...
    def compute_matvec(
        ...
        if strategy not in self.sub_processors:
            raise ValueError(f"Strategy '{strategy}' not available. Available: {list(self.sub_processors.keys())}")
    ...
    def _record_strategy_usage(self, strategy: str, computation_time: float, sparsity_ratio: float):
    ...
    def get_strategy_recommendations(self) -> Dict[str, Union[str, float]]:
        ...
        if best_strategy:
            ...
            confidence = 1.0 - (np.std(best_times) / np.mean(best_times)) if np.mean(best_times) > 0 else 0.0
    ...
    def compute_matvec_with_strategy(
        ...
        np.ndarray
            Result vector y.
        """
        return self.compute_matvec(x, decoding_depths, sparsity_pattern, force_strategy=strategy)
