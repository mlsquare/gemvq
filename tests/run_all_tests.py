...
def categorize_tests():
    ...
    analysis_tests = ["test_error_types.py", "test_error_trends.py"]
...
def main():
    ...
    parser.add_argument(
        "--test", type=str, help="Run a specific test (partial name match)"
    )
    parser.add_argument(
        "--category", type=str, help="Run tests from a specific category"
    )
    ...
    total_failed = sum(
        sum(1 for r in results if not r["success"]) for results in all_results.values()
    )
