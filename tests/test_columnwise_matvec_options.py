...
class TestColumnwiseMatVecOptions:
    ...
    def test_basic_matvec_computation(self):
        ...
        for name, processor in processors.items():
            ...
            error = np.linalg.norm(result - reference_result) / np.linalg.norm(
                reference_result
            )
    ...
    def test_sparsity_handling(self):
        ...
        error = np.linalg.norm(result_sparse - reference_sparse) / np.linalg.norm(
            reference_sparse
        )
        assert error < 1e-10
    ...
    def test_decoding_depths(self):
        ...
        for depth in depths:
            decoding_depths = [depth] * (
                self.matrix.shape[1] // 4
            )  # Assuming D4 lattice
    ...
    def test_lookup_table_strategies(self):
        ...
        for strategy in strategies:
            processor = create_lookup_table_processor(
                self.matrix, table_strategy=strategy
            )
