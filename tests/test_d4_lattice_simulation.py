...
def test_d4_lattice_simulation_zero_error():
    ...
    for i, original_point in enumerate(lattice_points):
        ...
        if i < 5:  # First 5 points should work perfectly
            ...
            assert (
                max_diff < 1e-8
            ), f"Point {i}: max difference = {max_diff}, expected < 1e-8"
        else:
...
def test_d4_lattice_quantization_consistency():
    ...
    for i, original_point in enumerate(lattice_points):
        ...
        assert (
            mse_between_methods < 1e-10
        ), f"Point {i}: Methods inconsistent, MSE = {mse_between_methods}"
...
def test_d4_lattice_known_issues():
    ...
    reconstructed_coarse = hq.decode_coarse_to_fine(
    ...
    assert (
        mse_regular < 1e-10
    ), f"Regular decode should work perfectly, got MSE = {mse_regular}"

    # Document that coarse-to-fine has issues
    if mse_coarse > 1e-10:
        print(
            f"⚠️  KNOWN ISSUE: decode_coarse_to_fine method has MSE = {mse_coarse} even at full depth"
        )
...
def test_d4_lattice_debug():
    ...
    for i, point in enumerate(d4_points):
        ...
        for depth in range(1, M + 1):  # 1 to M
            recon_depth = hq.decode_with_depth(
                b_list, T, with_dither=False, depth=depth
            )
