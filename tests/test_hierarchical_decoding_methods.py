...
from gemvq.quantizers.utils import calculate_mse, closest_point_Dn, get_d4
...
def test_all_decoding_methods():
    ...
    hq = HNLQ(G=G, Q_nn=closest_point_Dn, config=config, dither=np.zeros(4))
    ...
    try:
        reconstructed_coarse_full = hq.decode_coarse_to_fine(
            b_list, T, with_dither=False, depth=M - 1
        )
    ...
    try:
        reconstructed_depth_full = hq.decode_with_depth(
            b_list, T, with_dither=False, depth=M - 1
        )
    ...
    try:
        progressive_reconstructions = hq.decode_progressive(
            b_list, T, with_dither=False
        )
        mse_progressive_full = calculate_mse(
            test_point, progressive_reconstructions[-1]
        )  # Last (finest) level
...
def test_multiple_d4_points():
    ...
    for i, point in enumerate(d4_points):
        ...
        try:
            ...
            coarse_recon = hq.decode_coarse_to_fine(
                b_list, T, with_dither=False, depth=M - 1
            )
            ...
            regular_progressive_match = np.isclose(
                mse_regular, mse_progressive, rtol=1e-10
            )
