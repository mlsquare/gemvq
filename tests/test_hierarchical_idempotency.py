...
from gemvq.quantizers.utils import calculate_mse, closest_point_Dn, custom_round, get_d4
...
def test_idempotency_at_different_depths():
    ...
    # Create hierarchical quantizer
    config = HNLQConfig(lattice_type="D4", q=q, beta=beta, alpha=alpha, eps=eps, M=M)
    hq = HNLQ(config)
...
def test_progressive_idempotency():
    ...
    for i, (original, sample_beta) in enumerate(zip(samples, sample_betas)):
        ...
        print(
            f"Progressive reconstructions ({len(progressive_reconstructions)} levels):"
        )
