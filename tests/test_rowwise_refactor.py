...
def test_rowwise_refactor():
    ...
    config_full = HNLQConfig(
        lattice_type="D4", q=4, M=2, beta=1.0, alpha=1.0, eps=1e-8, decoding="full"
    )
    ...
    result_coarse = quantizer_coarse.decode_coarse_to_fine(
        b_list, T, with_dither=False, depth=1
    )
    result_progressive = quantizer_progressive.decode_progressive(
    ...
    row_gemv_full = RowWiseGEMV(matrix, lattice_type="D4", M=2, decoding="full")
    ...
    result_row_coarse_fine = row_gemv_coarse.multiply_coarse_to_fine(
        vector, max_level=1
    )
